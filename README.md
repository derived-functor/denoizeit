...
- `-c, --config` - Configuration file (default: `{YOUR_PLATFORM_CONFIG_DIR}/config.yaml`, i.e. `~/.config/denoizeit/config.yaml` for linux)
...
model_checkpoint:
  hf:
    repo_id: derived-functor/denoizeit-unet
    filename: model.pth
  local: file_path
```
...
> [!IMPORTANT]
> If `hf` option is provided, then `local` will be ignored
